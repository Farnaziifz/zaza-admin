...
import {queryDateClientFormat, queryDateServerFormat} from "@/core/constants/date.options";
...
export const convertDateArrayToDateString = (dateArray: Date[] | string[]) => {
    if (!_.isEmpty(dateArray)) {
        ...
        return `${
            new pd(convertDateFromPersianToGeorgian(dateArray[0]))
                .toLocale('fa')
                .format('YYYY/MM/DD')}, ${
            new pd(convertDateFromPersianToGeorgian(dateArray[1] as string))
                .toLocale('fa')
                .format('YYYY/MM/DD')}`
    }
...
export const getAllDaysInPeriodQuery = (type: reportPeriodType) => {
    ...
    return range.reduce((previousValue, currentValue, index) => previousValue + `Ranges[${index}].From=${currentValue.from}&Ranges[${index}].To=${currentValue.to}&`
        ,
