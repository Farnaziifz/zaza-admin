...
import { bannerList, banner } from '@/core/types/category.type'
import { initPageHandler } from '@/logics/specific/banner.handler'

const serverData: Ref<bannerList> = ref({
  total_pages: 0,
  next: 0,
  previous: 0,
  current_page: 0,
...
const columns: TableColumnType<banner>[] = [
  {
    title: 'عکس بنر',
    key: 'image',
    dataIndex: 'image',
...
const onChange: TableProps<bannerList>['onChange'] = async () => {
...
const goToAdd = () => {
  router.push({ name: 'banner-add' })
}
</script>

<template>
  <content-layout>
    <template #content-title>بنرهای دسته بندی</template>
